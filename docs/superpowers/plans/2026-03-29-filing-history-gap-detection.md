...
describe("detectAccountsGaps", () => {
  ...
  it("maps CH filed dates to computed expected period ends", () => {
    ...
    expect(result!.filedPeriodEnds.get(new Date("2020-03-31").getTime())).toEqual(new Date("2020-03-31"));
    expect(result!.filedPeriodEnds.get(new Date("2021-03-31").getTime())).toEqual(new Date("2021-03-31"));
  });
...
export function computeFirstPeriodEnd(
  ...
  let firstArd = new Date(Date.UTC(
    incorporationDate.getUTCFullYear(),
    ardMonth - 1,
    ardDay,
  ));
