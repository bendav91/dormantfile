...
```tsx
{
  isPreviewMode ? "Get started" : "Start filing";
}
<ArrowRight size={18} />;
```
...
```tsx
{
  isPreviewMode ? "Get started today" : "Start filing today";
}
<ArrowRight size={18} />;
```
...
<>
  ...
  <Link href={`/file/${companyId}/ct600?periodEnd=${periodEndISO}`} style={filingBtnStyle}>
    File
  </Link>
</>
```
...
```tsx
{
  !isPreviewMode && (ct600Filing.status === "failed" || ct600Filing.status === "rejected") && (
    <Link href={`/file/${companyId}/ct600?periodEnd=${periodEndISO}`} style={filingBtnStyle}>
      Retry
    </Link>
  );
