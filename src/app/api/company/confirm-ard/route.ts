 ...
 *
 * When confirmed: updates ardMonth/ardDay, recalculates outstanding accounts Filing
 * dates and deadlines. When dismissed: clears the flag for 90 days.
 */
export async function PATCH(req: NextRequest) {
  ...
  // Recalculate outstanding accounts filing dates and deadlines
  const outstandingAccountsFilings = await prisma.filing.findMany({
    where: {
      companyId,
      filingType: "accounts",
      status: "outstanding",
  ...
  let updated = 0;
  for (const filing of outstandingAccountsFilings) {
    // Compute new period end based on new ARD
    const newPeriodEnd = new Date(filing.periodStart);
    newPeriodEnd.setUTCFullYear(newPeriodEnd.getUTCFullYear() + 1);
    newPeriodEnd.setUTCMonth(newMonth - 1, newDay);
    if (newPeriodEnd.getTime() <= filing.periodStart.getTime()) {
      newPeriodEnd.setUTCFullYear(newPeriodEnd.getUTCFullYear() + 1);
    }
    ...
    await prisma.filing.update({
      ...
      data: {
        ...
        deadline: newAccountsDeadline,
      },
    });
    updated++;
  }

  return NextResponse.json({ success: true, filingsUpdated: updated });
}
