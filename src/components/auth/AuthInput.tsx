...
export function AuthInput({ label, helperText, error, id, type, ...props }: AuthInputProps) {
  ...
  return (
    <div>
      ...
      {helperText && !error && (
        <p id={helperId} className="text-xs mt-1.5" style={{ color: "var(--color-text-secondary)" }}>
          {helperText}
        </p>
      )}
      {error && (
        <p id={errorId} role="alert" className="text-xs mt-1.5 font-medium" style={{ color: "var(--color-danger-text)" }}>
          {error}
        </p>
      )}
    </div>
